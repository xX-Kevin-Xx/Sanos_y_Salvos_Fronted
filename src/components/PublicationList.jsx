...
const PublicacionList = ({ onVerDetalle }) => {
    ...
    <div className="mascota-list-container">
      <div className="mascota-list-header">
        ...
        <span className="mascota-list-count">
          {publicacionesFiltradas.length} {publicacionesFiltradas.length === 1 ? 'publicación' : 'publicaciones'}
        </span>
      </div>
          ...
          {publicacionesFiltradas.map((publicacion) => (
            <div key={publicacion.idPublicacion} className="mascota-card">
              <div className="mascota-card-image">
                <img 
                  src={publicacion.urlFoto || 'https://via.placeholder.com/300x200?text=Sin+Imagen'} 
                  alt={publicacion.titulo} 
                />
                <span className={`mascota-card-status ${
                  publicacion.tipoPublicacion === 'PERDIDA' ? 'status-perdido' : 'status-encontrado'
                }`}>
              ...
              <div className="mascota-card-content">
                ...
                <div className="mascota-card-detail">
                  <span>📍</span> {publicacion.direccionReferencia || 'Ubicación no especificada'}
                </div>
                ...
                <div className="mascota-card-contact">
                  ...
                  <span className="mascota-card-date">
                    {new Date(publicacion.fechaPublicacion).toLocaleDateString('es-CL')}
                  </span>
                </div>

                <button 
                  className="ver-detalle-btn"
                  onClick={() => onVerDetalle(publicacion.idPublicacion)}
                >
                  Ver más detalles
                </button>
              </div>
